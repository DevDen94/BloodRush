...
namespace MarsFPSKit
{
    ...
    public class Kit_PlayerBehaviour : MonoBehaviourPunCallbacks, IPunObservable
    {
        ...
        #endregion
        public void LocalDamagePlayer(float dmg)
        {
           /* if (main.assistManager)
            {
                main.assistManager.PlayerDamaged(main, botShot, idWhoShot, this, dmg);
           ...
           // ragdollForward = forward;
            ...
            if (photonView)
            {
                ...
                {
                    //Tell that player that we hit him
                    photonView.RPC("ApplyDamageNetworkForPlayer", RpcTarget.MasterClient, dmg);
                }
                else
                {
                    //Tell that player that we hit him
                    photonView.RPC("ApplyDamageNetworkForPlayer", photonView.Owner, dmg);
                }
            }
        }
        [PunRPC]
        public void ApplyDamageNetworkForPlayer(float dmg)
        {
            if (isController || isBot && PhotonNetwork.IsMasterClient)
            {
                ...
                //Relay to the assigned manager
                vitalsManager.ApplyDamage(this, dmg, true, 1, 1, Vector3.forward);
