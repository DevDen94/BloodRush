        ...
        public class Kit_ModernBullet : Kit_BulletBase
        {
            ...
            void OnHit(RaycastHit hit, Vector3 dir)
            {
                ...
                if (hit.transform.GetComponent<Kit_PlayerDamageMultiplier>() && main.currentGameModeType == 2)
                {
                    ...
                    //NewAi ai = hit.transform.GetComponent<NewAi>();
                ...
                else if(hit.transform.GetComponent<DetectPlayer>()|| hit.transform.gameObject.layer==9)
                {
                   
                    //Call
                 
                    //Check if we hit a player and if we are playing pvp
                    if (hit.transform.GetComponent<DetectPlayer>() || hit.transform.gameObject.layer == 9)
                    {
                        DetectPlayer pdm = hit.transform.GetComponent<DetectPlayer>();
                        //NewAi ai = hit.transform.GetComponent<NewAi>();
                        if (hit.transform.root.GetComponent<DetectPlayer>() || hit.transform.gameObject.layer == 9)
                        {
                           if (hit.transform.root.GetComponent<DetectPlayer>())
                            {
                                ...
                                //Apply local damage, sample damage dropoff via distance
                                hitPb.TakeDamage(settings.damage, settings.gameGunID);
