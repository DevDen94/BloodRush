...
namespace MarsFPSKit
{
    public class Kit_BotControlRuntimeData
    {
        ...
        public List<DetectPlayer> enemyPlayersAwareOffZombie = new List<DetectPlayer>();
    ...
    public class Kit_PlayerDefaultBotControl : Kit_PlayerBotControlBase
    {
        ...
        public override void WriteToPlayerInput(Kit_PlayerBehaviour pb)//Kit_PlayerBehaviour pb
        {
            Kit_BotControlRuntimeData kbcrd = pb.botControlsRuntimeData as Kit_BotControlRuntimeData;
            ...
            if (Time.time - 0.1f > kbcrd.lastRoutine)
            {
                ...
                if (Time.time - 5f > kbcrd.lastForceNewWaypoint || !kbcrd.playerToEngage && (kbcrd.nma.remainingDistance < 1f || !kbcrd.nma.hasPath) || kbcrd.playerToEngage && (kbcrd.nma.remainingDistance < 1f))
                {
                    ...
                    else
                    {
                        ...
                        {
                            int nearestWaypoint = Random.Range(0, pb.main.botNavPoints.Length);
                            float maxDistance = float.MaxValue;

                            for (int i = 0; i < pb.main.botNavPoints.Length; i++)
                            {
                                float dist = Vector3.Distance(kbcrd.playerToEngage.transform.position, pb.main.botNavPoints[i].transform.position);
                                if (dist < maxDistance)
                                {
                                    if (Vector3.Distance(pb.transform.position, pb.main.botNavPoints[i].transform.position) > 10f)
                                    {
                                        maxDistance = dist;
                                        int id = i;
                                        nearestWaypoint = id;
                            ...
                            //Set new waypoint
                            kbcrd.nma.SetDestination(pb.main.botNavPoints[nearestWaypoint].position + (Random.insideUnitSphere * Random.Range(1, 3f)));
                        }
                    }
                }
            }
        ...
        void CheckForEnemies(Kit_PlayerBehaviour pb, Kit_BotControlRuntimeData kbcrd)
        {
            ...
            for (int i = 0; i < possiblePlayers.Length; i++)
            {
                ...
                DetectPlayer pnb = possiblePlayers[i].transform.root.GetComponent<DetectPlayer>();//kitplayerbehaviour.ROOT
               
                if (pnb && pnb != pb)
                {
                   
                    if (CanSeeZombie(pb, kbcrd, pnb))
                    {
                       
                        if (isEnemyZombie(pb, kbcrd, pnb))
                        {
                           
                            if (!kbcrd.enemyPlayersAwareOffZombie.Contains(pnb))
                            {
                                //Add to our known list
                                kbcrd.enemyPlayersAwareOffZombie.Add(pnb);
        ...
        void UpdatePlayerToEngage(Kit_PlayerBehaviour pb, Kit_BotControlRuntimeData kbcrd)
        {
            ...
            kbcrd.enemyPlayersAwareOffZombie = kbcrd.enemyPlayersAwareOffZombie.OrderBy(x => Vector3.Distance(pb.transform.position, x.transform.position)).ToList();
           
            for (int i = 0; i < kbcrd.enemyPlayersAwareOffZombie.Count; i++)
            {
                if (i < kbcrd.enemyPlayersAwareOffZombie.Count)
                {
                    if (CanSeeZombie(pb, kbcrd, kbcrd.enemyPlayersAwareOffZombie[i]))
                    {
        ...
        void CheckForzOMBIES(Kit_PlayerBehaviour pb, Kit_BotControlRuntimeData kbcrd)
        {
            Collider[] possiblePlayers = Physics.OverlapBox(pb.playerCameraTransform.position + pb.playerCameraTransform.forward * (spottingMaxDistance / 2), spottingBoxSize, pb.playerCameraTransform.rotation, spottingLayer.value);
            ...
            {
                //Check if it is a player
                DetectPlayer pnb = possiblePlayers[i].transform.GetComponent<DetectPlayer>();//kitplayerbehaviour.ROOT
                Debug.LogError("PB" + possiblePlayers[i]);
                if (pnb && pnb != pb)
                {
                    Debug.LogError("PB");
                    if (CanSeeZombie(pb, kbcrd, pnb))
                    {
                        Debug.LogError("SeeZombie");
                        if (isEnemyZombie(pb, kbcrd, pnb))
                        {
                            Debug.LogError("EnemyZombie");
                            if (!kbcrd.enemyPlayersAwareOffZombie.Contains(pnb))
                            {
                                //Add to our known list
                                kbcrd.enemyPlayersAwareOffZombie.Add(pnb);
                                Debug.LogError("AddZombie");
                                //Call spotted
                                if (pb.voiceManager)
                                {
                                    // pb.voiceManager.SpottedEnemy(pb, pnb);
        ...
        bool CanSeeZombie(Kit_PlayerBehaviour pb, Kit_BotControlRuntimeData kbcrd, DetectPlayer enemyPlayer)
        {
          
            if (enemyPlayer)
            {
                RaycastHit hit;
                Vector3 rayDirection = enemyPlayer.transform.position - new Vector3(0, 0.2f, 0f) - pb.transform.position;

                if ((Vector3.Angle(rayDirection, pb.transform.forward)) < spottingFov)
                {
                    Debug.DrawLine(pb.transform.position, rayDirection,Color.red);
                    if (Physics.Raycast(pb.transform.position, rayDirection, out hit, spottingRayDistance, spottingCheckLayers.value))
                    {
                        //Debug.Log("RayCastHitedName : "+hit.collider.gameObject.name);
        ...
        bool isEnemyZombie(Kit_PlayerBehaviour pb, Kit_BotControlRuntimeData kbcrd, DetectPlayer enemyPlayer)
        {
            if (pb)
            {
                return true;
          ...
          private List<GameObject> players = new List<GameObject>();
                ...
                void UpdateFunc()
                {
                    if (Wait)
                    {
                        ...
                        {
                            float distanceToPlayer = Vector3.Distance(this.transform.position, nearestPlayer.transform.position);
