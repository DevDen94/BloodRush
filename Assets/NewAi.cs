...
public class NewAi : MonoBehaviour
{
	...
	void OnEnable()
	{
		...
		if (!spawned)
		{//if spawned, SpawnNPC function will be called from NPCSpawner.cs. Otherwise, spawn now.	
			StopAllCoroutines();
		...
		else
		{
			while (true)
			{
				...
				else
				{
					...
					{
						NPCRegistryComponent.FindClosestTargetMultiPlayer(myTransform.gameObject, this, myTransform.position, attackRangeAmt, factionNum);
					}
				}
	...
	[SerializeField] private float aggroRange = 15f; // Distance within which a player is considered "near"
	...
	{/*
		...
		foreach (Collider collider in colliders)
		{
			...
			{
				// The player is near the zombie
				string playerName = collider.GetComponent<PhotonView>().Owner.NickName;
				Debug.Log("Player " + playerName + " is near the zombie!");
			}
		}*/
		/*//yield return new WaitForSeconds(0.1f);
		//if (!PhotonNetwork.IsConnectedAndReady) { return; } // Only run on active network

		nearestPlayer = null; // Reset nearest player each frame

		// Loop through all connected players
		foreach (Player player in PhotonNetwork.PlayerList)
		{
			
			//if (player == PhotonNetwork.LocalPlayer) { continue; } // Skip local player
            ...
            if (playerObject.GetComponent<Kit_PlayerBehaviour>())
            {
				float distance = Vector3.Distance(transform.position, playerObject.transform.position);
				if (distance <= aggroRange && (nearestPlayer == null || distance < Vector3.Distance(transform.position, nearestPlayer.transform.position)))
				{
				...
				Debug.LogError("aya" + distance);
			}
			

			// Check if player is within aggro range and closer than any other player found so far
			

			//PhotonNetwork.LocalPlayer.SetCustomProperties(new ExitGames.Client.Photon.Hashtable { { "NearestPlayer", nearestPlayer.PhotonView.ViewID } });
			

		}
		//Debug.LogError(playerTransform);
		//StartCoroutine(GetTarget());*/
