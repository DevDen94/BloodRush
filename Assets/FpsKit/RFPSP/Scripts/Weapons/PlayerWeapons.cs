		...
		&& !FPSWalkerComponent.holdingObject){
			...
			if(!sprintSwitching){
				...
				if(Time.timeScale > 0.0f){
					...
					if ((InputComponent.mouseWheel < 0 && !CameraControlComponent.rotating /*&& !CameraControlComponent.thirdPersonActive*/) 
					|| InputComponent.selectPrevPress 
					|| InputComponent.xboxDpadLeftPress){//mouse wheel down or previous weapon button pressed
					...
					}else if((InputComponent.mouseWheel > 0 && !CameraControlComponent.rotating /*&& !CameraControlComponent.thirdPersonActive*/) //mouse wheel up
	...
	public void DropWeapon ( int weapon){
		...
		if(weaponOrder[weapon].GetComponent<WeaponBehavior>().weaponDropObj){
			...
			if(!CameraControlComponent.thirdPersonActive){
				weaponObjDrop = Instantiate(weaponOrder[weapon].GetComponent<WeaponBehavior>().weaponDropObj, mainCamTransform.position + playerObj.transform.forward * 0.25f + Vector3.up * -0.25f, mainCamTransform.rotation) as GameObject;
			}else{
				weaponObjDrop = Instantiate(weaponOrder[weapon].GetComponent<WeaponBehavior>().weaponDropObj, playerObj.transform.position + playerObj.transform.forward * 0.25f + Vector3.up * -0.25f, mainCamTransform.rotation) as GameObject;
			}
