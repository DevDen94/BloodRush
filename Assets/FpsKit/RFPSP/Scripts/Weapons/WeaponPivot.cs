...
public class WeaponPivot : MonoBehaviour {
	...
	void Update () {
		...
		if(Time.timeScale > 0.0f && Time.smoothDeltaTime > 0.0f && Time.deltaTime > 0.0f ){
			...
			if (sumX + x < -WeaponBehaviorComponent.horizontalDZ) {
				...
				HorizontalLook();//move camera horizontally when weapon is at max horizontal rotation
			...
			if (sumY + y < -WeaponBehaviorComponent.verticalDZUpper) {
				...
				VerticalLook();//move camera vertically when weapon is at max vertical rotation            
	...
	private void HorizontalLook () {//move camera horizontally when weapon is at max horizontal rotation
	...
	private void VerticalLook () {//move camera vertically when weapon is at max vertical rotation
		SmoothMouseLookComponent.rotationY += InputComponent.lookY * SmoothMouseLookComponent.sensitivityAmt * Time.timeScale;
	}

	//play weapon animation for jumping 
	...
	//play weapon animation for Landing
